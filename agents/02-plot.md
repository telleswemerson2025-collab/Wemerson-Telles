...
terminal VantageNode** (com a marca d'água nativa), em range de **3 anos**, enquadrado como cabeçalho +
...
- `docs/data-sourcing.md` — a recipe de captura (range 3Y, enquadramento cabeçalho + gráfico, sem
...
1. Obter o **screenshot real** da métrica na VantageNode (via Claude Chrome, na aba logada do Mr. G),
   **em 3 anos** e enquadrado conforme a recipe.
...
4. (Opcional) emitir `02_chart_meta.json` com a **proveniência** (métrica, permalink, range 3Y, `as_of`).
...
- Enquadramento: **cabeçalho (logo + breadcrumb) + gráfico**, sem barra de ferramentas nem aba lateral;
  marca d'água visível; range **3 anos**.
