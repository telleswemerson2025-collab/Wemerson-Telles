...
terminal VantageNode** (com a marca d'água nativa), em range de **5 anos (botão 5Y)**, enquadrado como cabeçalho +
...
- `docs/data-sourcing.md` — a recipe de captura (range 5Y, enquadramento cabeçalho + gráfico, sem
...
1. Obter o **screenshot real** da métrica na VantageNode (via Claude Chrome, na aba logada do Mr. G),
   **em 5 anos (botão 5Y)** e enquadrado conforme a recipe.
...
4. (Opcional) emitir `02_chart_meta.json` com a **proveniência** (métrica, permalink, range 5Y, `as_of`).
