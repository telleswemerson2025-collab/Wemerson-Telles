...
export function varrer({ varredura, hoje, camada5 = null, anterior = null }) {
  ...
  for (const s of SERIES) {
    ...
    const conf = confianca(s.inicioSerie, hoje);
    lidos.set(s.n, {
  ...
  for (const camada of [2, 3, 4]) {
    ...
    if (presentes.length === daCamada.length) {
      camadas.set(camada, {
        posicao: media(presentes.map((x) => x.posicao)),
  ...
  return {
    ...
    camadasForaDaConta: [1, 2, 3, 4, 5].filter((c) => !camadas.has(c))
      .map((c) => ({ camada: c, motivo: c === 5 ? (camada5?.motivo ?? 'sem carteira ativa') : 'camada incompleta' })),
 ...
 */
export function filtroDeHorizonte(ativo, j = {}) {
  const reprovas = [], pendentes = [];
  if (['BTC', 'ETH'].includes(ativo)) {
    // alínea (b): BTC e ETH passam por definição
  } else if (typeof j.ciclosCompletos === 'number') {
    if (j.ciclosCompletos < 1) reprovas.push('(b) não atravessou um ciclo completo');
  } else pendentes.push('(b) ciclos completos');

  for (const [chave, alinea] of [['liquidezSuficiente', '(a) liquidez'], ['teseSemEventoDatado', '(c) tese sem evento datado'], ['semAlavancagemOuContraparte', '(d) sem alavancagem, sintético ou contraparte concentrada']]) {
    if (typeof j[chave] !== 'boolean') pendentes.push(alinea);
    else if (!j[chave]) reprovas.push(alinea);
  }

  if (reprovas.length) return { veredito: 'reprovado', motivo: reprovas.join(' · ') };
  if (pendentes.length) return { veredito: 'pendente', motivo: `aguarda julgamento humano: ${pendentes.join(' · ')}` };
  return { veredito: 'aprovado', motivo: 'passa nas quatro alíneas do Filtro de Horizonte' };
}

export function varreduraDaCRM({ registro, carteira, composicao, hoje, julgamentos = {} }) {
  ...
  const filtrados = incluidos.map((ativo) => {
    const r = filtroDeHorizonte(ativo, julgamentos[ativo]);
