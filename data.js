...
// PERSISTÊNCIA
// =====================
// Gera cobrança de arbitragem (R$30, pendente) POR JOGO para cada atleta convocado.
// Modelo por jogo: o mesmo atleta paga R$30 em cada jogo que é convocado.
// catNome='Sub-13'/catKey; sigs=siglas; jogoId=id do jogo; jogoLabel='vs Adversário · data'.
function gerarArbitragemConvocados(catNome, sigs, jogoId, jogoLabel){
  ...
  const jid = jogoId || 'avulso';
  sigs.forEach(sig => {
    // Dedup por JOGO+atleta: um atleta pode ter várias cobranças (uma por jogo)
    if(!lista.find(a => a.sig === sig && (a.jogoId||'avulso') === jid)){
      lista.push({sig, jogoId: jid, jogo: jogoLabel || '', pago: false, status: 'pendente'});
    }
  });
}
